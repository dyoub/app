...
{
    public class UpdateSalePaymentsContext : TenantContext
    {
        ...
        public UpdateSalePaymentsContext() : base(1, DbConnectionFactory.CreateTransient())
        {
            Tenant tenant = Tenants.Add(TenantFactory.Tenant());
            Store store = Stores.Add(StoreFactory.Store(tenant));
            PaymentMethod paymentMethod = PaymentMethods.Add(PaymentMethodFactory.PaymentMethod(tenant));
            SaleOrder saleOrder = SaleOrders.Add(SaleOrderFactory.SaleOrder(store));
        ...
        public bool SalePaymentsWasUpdated()
        {
